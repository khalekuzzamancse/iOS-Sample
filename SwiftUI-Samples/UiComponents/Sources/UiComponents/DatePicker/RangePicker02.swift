...
private struct _DateRangePickerDialog: View {
    ...
    var body: some View {
        ZStack {
            _DialogBackground { withAnimation { showDialog = false } }
...
private struct _DateRangePickerView: View {
    @Binding var range: ClosedRange<Date>
        ...
        VStack(spacing: 12) {
            DatePicker(
                ...
                selection: Binding(
                    ...
                    set: { range = $0...max($0, range.upperBound) }
                ),
                in: validRange,
            ...
            DatePicker(
                ...
                selection: Binding(
                    ...
                    set: { range = min($0, range.lowerBound)...$0 }
                ),
                in: validRange,
                displayedComponents: [.date]
            )
...
private struct _DialogBackground: View {
    var onTap: () -> Void
...
private struct _DialogButtons: View {
    var onCancel: () -> Void
    var onDone: () -> Void
